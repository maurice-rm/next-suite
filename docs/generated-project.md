...
| `features/api/orpc/core`                   | the API layer is oRPC                                  | `src/instrumentation.ts`, `src/app/providers.tsx`, `src/app/api/rpc/[[...rest]]/route.ts`, `src/orpc/{client.server.ts,client.ts,index.ts,query-client.ts,server.tsx}`, `src/orpc/routers/app-router.ts`, `src/features/health/server/router.ts`, plus an `.env.example` fragment |
