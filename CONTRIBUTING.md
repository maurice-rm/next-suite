...
That token goes in through the `github-token` input and nowhere else. `changesets/action` refuses to start when a `GITHUB_TOKEN` environment variable is set to anything but the same value, and it already injects the token it was given into the `version` and `publish` scripts — which is where `@changesets/changelog-github` picks it up.

npm authentication uses no secret at all. The package is configured on npmjs.com with a [trusted publisher](https://docs.npmjs.com/trusted-publishers) naming this repository and `release.yml`, so the `npm publish` that `changeset publish` shells out to exchanges the job's OIDC token for a short-lived registry credential itself. `id-token: write` in the job's `permissions` is what makes that token available — without it the publish fails.

Two consequences worth knowing before touching this. Nothing may set an npm auth token in the release job: `npm` falls back to a token when the OIDC exchange fails, and because that exchange is written never to throw, a leftover credential turns a broken trusted-publisher configuration into a misleading `E404` instead of a clear `ENEEDAUTH`. And provenance is deliberately _not_ forced — npm turns it on by itself for a public package in a public repository, whereas setting `NPM_CONFIG_PROVENANCE` explicitly bypasses that visibility check and would hard-fail the publish if either ever went private.

The trusted publisher names the workflow file, so renaming `release.yml` or moving the publish step into another workflow breaks releases until the configuration on npmjs.com is recreated to match — existing ones cannot be edited.

Two smaller details in the same workflow. The checkout sets `persist-credentials: false` because `changesets/action` defaults to `push-with-git-cli: false`, committing through the API token rather than `git push`, so the checkout never needs credentials left behind. And the job runs on the Node 24 LTS line: it was pinned to 22.22 for a while because Node 22.23.0's fix for CVE-2026-48931 broke `node-fetch` keep-alive and surfaced as a "Premature close" from the changelog plugin ([nodejs/node#63989](https://github.com/nodejs/node/issues/63989)). If that error ever comes back, pin to the last 24.x without the fix.
