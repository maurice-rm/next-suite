...
npm authentication is separate and does not go through the action at all: v2 stopped writing an `.npmrc` from `NPM_TOKEN`, so the setup action takes a `registry-url` and lets `actions/setup-node` write one that reads `NODE_AUTH_TOKEN`. That is why the publish step passes the `NPM_TOKEN` secret as `NODE_AUTH_TOKEN` — under any other name npm never sees it and `changeset publish` fails with `ENEEDAUTH`.
