...
`release.yml` needs one note. GitHub deliberately starts no workflow for an event caused by the built-in `GITHUB_TOKEN`, which would otherwise let a workflow retrigger itself. A "Version Packages" pull request opened with that token therefore carries no checks at all — and `main` requires `Verify` and both `Test` jobs, so the release pull request can never satisfy them. `changesets/action` is given `secrets.RELEASE_PAT` for that reason: a fine-grained token scoped to this repository with `Contents` and `Pull requests` write access, whose events count as a user's and do start workflows. The expression falls back to `GITHUB_TOKEN`, so releases still work without the secret — the release pull request just has to be closed and reopened by hand to make the checks run.
