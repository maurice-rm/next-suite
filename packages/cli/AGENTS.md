...
- **`release.yml`** (pushes to `main`): `changesets/action` opens/updates a `chore(release): version packages` PR; merging it runs `changeset version` and **`changeset publish`** — tagging the release, creating a GitHub Release from the changelog, and publishing to npm under `latest`. Requires the `NPM_TOKEN` secret, passed as `NODE_AUTH_TOKEN` so the `.npmrc` written by `actions/setup-node` picks it up, and npm provenance is on, which needs the repository to stay public.
