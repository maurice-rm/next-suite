...
- **`release.yml`** (pushes to `main`): `changesets/action` opens/updates a `chore(release): version packages` PR; merging it runs `changeset version` and **`changeset publish`** — tagging the release, creating a GitHub Release from the changelog, and publishing to npm. The repo is in pre mode, so packages ship under the `beta` dist-tag and a follow-up step points `latest` at that version. Requires the `NPM_TOKEN` secret. `NPM_CONFIG_PROVENANCE` is off because provenance needs a public repo.
